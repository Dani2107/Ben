...
#adds dawn
df_wdm131[,8:12]<-getSunlightTimes(date = df_wdm131$LocalDate, lat = 0.2922, lon = 36.8980, , keep = c("dawn"), tz = "Africa/Nairobi")
#deletes columns we don't need
df_wdm131<-df_wdm131[,-c(8:10,12)]

#same for dusk
df_wdm131[,9:13]<-getSunlightTimes(date = dates, lat = 0.2922, lon = 36.8980, , keep = c("dusk"), tz = "Africa/Nairobi")
df_wdm131<-df_wdm131[,-c(9:11,13)]

#change local time to time
df_wdm131$LocalTime<-chron(times=df_wdm131$LocalTime)

#sum activity across two axes and name column
df_wdm131[,10]<-df_wdm131[,5]+df_wdm131[,6]
colnames(df_wdm131)[10] <- "Activity.Both"
...
for(i in 1:nrow(df_wdm131)){
  if(sum(df_wdm131[i+1,10],df_wdm131[i+2,10],df_wdm131[i+3,10])==0 && df_wdm131[i,10]>0 && df_wdm131[i+1,1]==df_wdm131[i,1]){
    df_wdm131[i+1,14]<-"Stop"          # && df_wdm131[i+1,1]==df_wdm131[i,1]) this bit controls for same dog
  }
  if(df_wdm131[i+1,10]>0 && df_wdm131[i,10]==0 && df_wdm131[i+1,1]==df_wdm131[i,1]){
...
df_wdm131<-df_wdm131[,c(1,2,3,4,5,6,7,10,8,9)]    # re-ordering columns so SSDateTime next to SumAct

StartStops2<-which(df_wdm131[,5]>0)  # making vector of all rows with starts and stops

for(i in 2:length){
  if(i %%2 ==0){
    df_wdm131[StartStops2[i],9]<-sum(df_wdm131[StartStops2[i]:StartStops2[i+1],4])
  }         # working out sum of activity between start and stop
}

df_wdm131_nona<-df_wdm131[(df_wdm131$Bout)!=0, ]
length2<-nrow(df_wdm131_nona)

df_wdm131_nona[,10]<-NA
...
write.table(df_wdm131_nona,"~/Desktop/WDM131_SS.txt",sep="\t",row.names=FALSE)
