...
first<-min(dog.data$LocalDate[is.finite(dog.data$LocalDate)])
last<-max(dog.data$LocalDate[is.finite(dog.data$LocalDate)])
dates<-seq(first,last, by="days")
df.dates<-data.frame(dates)
df.dates[,2]<-NA
df.dates[,3:7]<-getSunlightTimes(date = dates, lat = 0.2922, lon = 36.8980, , keep = c("dawn","dusk"), tz = "Africa/Nairobi")
df.dates[,8:13]<-getMoonTimes(date = dates, lat = 0.2922, lon = 36.8980, , keep = c("rise","set"), tz = "Africa/Nairobi")
df.dates$Date_2<-df.dates$dates+1
df.dates[,14:19]<-getSunlightTimes(date = df.dates$Date_2, lat = 0.2922, lon = 36.8980, , keep = c("dawn","dusk"), tz = "Africa/Nairobi")
df.dates[,20]<-as.POSIXct(df.dates[1,1], tz = "Africa/Nairobi")
df.dates[,21]<-as.POSIXct(df.dates[1,1], tz = "Africa/Nairobi")
df.dates[,22]<-0
df.dates<-df.dates[,-c(8:10,13,15,16,19)]

View(df.dates)

#fill in the NA's with the correct date
for(i in 1:nrow(df.dates)){
  if(is.na(df.dates[i,9])==TRUE){
    df.dates[i,9]<-df.dates[i+1,9]
  }
  if(is.na(df.dates[i,8])==TRUE){
    df.dates[i,8]<-df.dates[i+1,8]
  }
}

#write the correct moonrise and moonset for the night after that date
for(i in 1:nrow(df.dates)){
  if(df.dates[i,8]<df.dates[i,6]){
    df.dates[i,13]<-df.dates[i+1,8]
  } else {
    df.dates[i,13]<-df.dates[i,8]
  }
}

for(i in 1:nrow(df.dates)){
  if(df.dates[i,9]<df.dates[i,13]){
    df.dates[i,14]<-df.dates[i+1,9]
  } else {
    df.dates[i,14]<-df.dates[i,9]
  }
}
...
for(i in 1:nrow(df.dates)){
  ...
  if(df.dates[i,13]>df.dates[i,6]&df.dates[i,13]<df.dates[i,7]&df.dates[i,14]<df.dates[i,7]&df.dates[i,14]>df.dates[i,6]){
    df.dates[i,15]<-0
  }
  ...
  if(df.dates[i,13]>df.dates[i,6]&df.dates[i,13]<df.dates[i,7]&df.dates[i,14]>df.dates[i,7]&df.dates[i,14]<df.dates[i,11]){
    df.dates[i,15]<-difftime(df.dates[i,14],df.dates[i,7], units = "hours")
  }
  ...
  if(df.dates[i,13]>df.dates[i,6]&df.dates[i,13]<df.dates[i,7]&df.dates[i,14]>df.dates[i,7]&df.dates[i,14]>df.dates[i,11]){
    df.dates[i,15]<-difftime(df.dates[i,11],df.dates[i,7], units = "hours")
  }
  ...
  if(df.dates[i,13]<df.dates[i,11]&df.dates[i,13]>df.dates[i,7]&df.dates[i,14]>df.dates[i,7]&df.dates[i,14]<df.dates[i,11]){
    df.dates[i,15]<-difftime(df.dates[i,14],df.dates[i,13], units = "hours")
  }
  ...
  if(df.dates[i,13]<df.dates[i,11]&df.dates[i,13]>df.dates[i,7]&df.dates[i,14]>df.dates[i,11]){
    df.dates[i,15]<-difftime(df.dates[i,11], df.dates[i,13], units = "hours")
  }
}

for(i in 1:nrow(df.dates)){
  if(is.na(df.dates[i,15])==TRUE){
    df.dates[i,15]<-0
  }
}
